...
RSpec.describe User, type: :model do
  ...
  describe 'accept_friend_request' do
    it 'accepts other users friend requests' do
      ...
      expect(another_user.passive_friendships.first.status).to eq('accept')
      expect(user.active_friendships.first.status).to eq('accept')
    end
  end
  ...
  describe 'friends' do
    ...
    it 'does not list users that have pending status' do
      user = create(:user)
      another_user = create(:user)
      user.send_friend_request_to(another_user)
      expect(user.friends.count).to eq be_zero
    end
  end
  ...
  describe 'friend?' do
    ...
    it 'returns false if the user is not a friend of another user' do
      user = create(:user)
      another_user = create(:user)
      user.send_friend_request_to(another_user)
